...
#define get_MbusChannelNumber( )    getChannel_Mbus( )

#define EEPROM_I2C_MultipleWrite(Buffer,WriteAddr,NumByteToWrite)  EEROM_Write(WriteAddr,Buffer,NumByteToWrite) 

#define EEPROM_I2C_MultipleRead(Buffer,ReadAddr,NumByteToRead)     EEROM_Read(ReadAddr,Buffer, NumByteToRead) 
/*********************************************************************************************************
数据类型定义
*********************************************************************************************************/
...
struct KindgDaSPCPSendBuff{
  union{
    ...
    struct {
      ...
      unsigned char AckEN : 1;     //是否需要应答  1: 需要  0: 无需应答
